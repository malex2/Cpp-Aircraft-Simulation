...
void ModelMap::addLogVar(std::string name, const double *var, const mapType *mappings, int nMappings)
{
    for (int curMap = 0; curMap < nMappings; curMap++)
    {
        if (mappings[curMap] == printVar)
        {
            ...
            printMapping.insert( std::pair<int, const double*> (iPrintNames, var) );
        ...
        if (mappings[curMap] == saveVar)
        {
            ...
            saveMapping.insert( std::pair<int, const double*> (iSaveNames, var) );
        ...
        if (mappings[curMap] == plotVar)
        {
            ...
            plotMapping.insert( std::pair<int, const double*> (iPlotNames, var) );
...
const double* ModelMap::getLogVar(iLog itr)
{
