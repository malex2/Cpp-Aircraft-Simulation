...
public:
    ...
    // Getters
    AngleType<double>* getAeroEuler(void) { return aeroEuler; };
    double* getAeroMatrix(void)           { return *aeroMatrix; }
    
    // Setters
    void setAeroEuler(AngleType<double>* angles_in);
...
protected:
    ...
    // Variables
    AngleType<double> aeroEuler[3]; // {0, alpha, -beta}
    AngleType<double> alpha, beta;
    double refArea;
    
    // Print variables
    double aeroForce[3];       // Lift, Drag, Side Force
    double bodyForceCoeff[3];  // CX, CY, CZ
    double bodyMomentCoeff[3]; // Cl, Cm, Cn
    double betaPrint, alphaPrint;
    ...
    double aeroMatrix[nCoeff][nDeriv];
    double coeffs[nCoeff];
    double inputs[nDeriv];
    
    double Cdarray[nDeriv];
    double CLarray[nDeriv];
    double CXarray[nDeriv];
    double CYarray[nDeriv];
    double CZarray[nDeriv];
    double Clarray[nDeriv];
    double Cmarray[nDeriv];
    double Cnarray[nDeriv];
...
private:
    ...
    const double reynoldsVec[nReynolds] = {72492.35967, 79016.24268, 84086.99144, 91413.98715, 106452.9941, 126196.9146, 177280.9713, 275866.5754};
    ...
    const double AeroTable_LongitudinalT[nReynolds][10] {
        //CLo, Cdo ,    CXu    ,   CXa   ,    CZu     ,  CLa  ,  CLq  ,    Cmo    ,   Cma   ,   Cmq
        {0.31, 0.04,    0.0    , 0.58102 ,    0.0     , 3.7883, 6.3436, 0.12744635, -1.5562 , -9.4643},
        {0.31, 0.04,    0.0    , 0.45587 ,    0.0     , 3.9413, 6.4242, 0.0820698 , -1.3519 , -9.3511},
        {0.31, 0.04,    0.0    , 0.39016 ,    0.0     , 4.0162, 6.4782, 0.06193474, -1.2391 , -9.2764},
        {0.31, 0.04,    0.0    , 0.32141 ,    0.0     , 4.0878, 6.5454, 0.04438665, -1.1164 , -9.1863},
        {0.31, 0.04,    0.0    , 0.23267 ,    0.0     , 4.1653, 6.6501, 0.01808996, -0.95027, -9.0499},
        {0.31, 0.04,    0.0    , 0.16717 ,    0.0     , 4.2072, 6.7408, 0.02221337, -0.82203, -8.9323},
        {0.31, 0.04,    0.0    , 0.092156,    0.0     , 4.2345, 6.8574, 0.02221371, -0.67029, -8.7783},
        {0.31, 0.04,    0.0    , 0.048239,    0.0     , 4.2385, 6.9307, 0.02591836, -0.57985, -8.6788}
    };
    double AeroTable_Longitudinal[10][nReynolds];
    
    enum lateralCoefficients {lookupCYb, lookupCYp, lookupCYr, lookupClb, lookupClp, lookupClr, lookupCnb, lookupCnp, lookupCnr};
    const double AeroTable_LateralT[nReynolds][9] {
    ...
    double rudderTable[3][nDr];
};

// **********************************************************************
// Quadcopter Aerodynamic Model
// **********************************************************************
class QuadcopterAeroModel : public AeroModelBase
{
public:
    // Constructor
    QuadcopterAeroModel(ModelMap *pMapInit, bool debugFlagIn = false);
